...
def insertDonor(name,age,gender,address):
	insert1 = "INSERT INTO donor(name,age,gender,address) VALUES('"+name+"','"+age+"','"+gender+"','"+address+"')"
	try:
...
def retrieve(bg):
	request="SELECT * FROM BLOOD where bloodgroup='"+bg+"'"
...
def donordetails():
	...
	b1=Button(root,text="Submit",command=lambda : insertDonor(e1.get(),e2.get(),e3.get(),e4.get())).place(x=40,y=300)
...
def blooddetails():
	...
	l2=Label(root,text="Platelet count (in 100 thousands):",font="Helvetica 12").place(x=40,y=80,w=250,h=20)
